...
class BiometricIDAuth {
    ...
    lazy var availabelTouchId: Bool = {
        if #available(iOS 11.0, *) {
    ...
    func requestPermission(completion: ((_ granted: Bool) -> ())?) {
        if self.hasBiometrics == false {
            completion?(false)
            return
        }
        context.evaluatePolicy(.deviceOwnerAuthenticationWithBiometrics, localizedReason: "i know") { (granted, error) in
            completion?(granted)
        }
    }
}
